...
pub fn setup_draw(window: &gtk::ApplicationWindow) -> Result<DrawingArea, String> {
    ...
    let setup_core = draw_core(size, map_size, f_map_size)?;
    ...
    darea.set_draw_func(glib::clone!(@weak window =>move |darea, context, _, _| {
        ...
        let res = setup_core(context, 0.5).and_then(|_| {
