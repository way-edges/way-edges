...
use config::widgets::wrapbox::BoxConfig;
use config::Config;
...
impl BoxCtx {
    fn new(config: &Config, box_conf: &mut BoxConfig, darea: &DrawingArea) -> (Self, BoxExpose) {
        ...
        let ol = match box_conf.outlook.take().unwrap() {
            config::widgets::wrapbox::Outlook::Window(c) => outlook::window::BoxOutlookWindow::new(
                c,
...
pub fn init_widget(window: &mut WindowContext, conf: Config, mut box_conf: BoxConfig) {
    let edge = conf.edge;
...
fn init_boxed_widgets(box_conf: &mut BoxConfig, expose: BoxExpose) -> GridBox<BoxedWidgetRc> {
    ...
    ws.into_iter().for_each(|w| {
        ...
        .inspect_err(|e| {
            util::notify_send("Way-edges boxed widgets", e.as_str(), true);
            log::error!("{e}");
        });
    });
